...
| `WSS.record.releases` | **The release list.** One entry per milestone: the version it intends to ship as, which goals it comprises, and whether it is marked completed. Plus the declaration, where one has been made, that milestones have ended. | Goal prose or task breakdowns — it *cites* the goals a milestone comprises rather than restating them. The claim that a version *shipped* — a tag is the only proof of that. |
