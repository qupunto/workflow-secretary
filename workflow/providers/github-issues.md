...
reads here. What was built is recorded in `record.decisions` and
`record.changelog`, exactly as with a file.
...
the exact thing this exists to prevent. `doctor.sh` checks the provider resolves
