...
[`sweep-tracker`](writers/sweep-tracker.md) and **read by everyone else**
— which is what keeps the one-writer invariant in [`ownership.md`](ownership.md)
literally true rather than needing an exception for shared state.

Who may write what is [`ownership.md`](ownership.md); what each *record* holds is
[`record-contract.md`](record-contract.md). This file is about **shape**, and
about a file that is deliberately not a record.

## It is a cache, not a record

`.claude/sweeps.json` by default; the manifest key is `sweeps`. **Gitignored, and
...
| Field | Meaning |
|---|---|
| `sweep` | Contract version. A skill that does not recognise it sweeps in full rather than guessing — the same reason `manifest` is required in [`manifest.md`](manifest.md) |
...
Generalised from [`audit-coverage.md`](audit-coverage.md), which states them for
...
- **Prose, reasoning, or task lists.** Those are `record.decisions` and
  `record.todo`, and a cache that accumulates them becomes a second record that
