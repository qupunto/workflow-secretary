...
### 2. Validate every key against `manifest.md`

Three checks, and all three are cheap:

- **The key is documented.** A key with no row in
  [`manifest.md`](../manifest.md) is dead config — nothing reads it,
...
- **The value resolves.** Every path must exist at the moment it is written, and
  ...
  **`record.todo` may be a provider object rather than a path**, and then this
  check is a different one — there is no file to stat. Key on the presence of a
  `provider` key, not on the value being an object (`record.tooling` is an object
  ...
  what `doctor.sh` checks, and the two must agree. An unimplemented provider or a
...
Ordered as [`manifest.md`](../manifest.md) presents them, so two
...
may still be whitelisted in `doctor.sh`'s `KNOWN_KEYS`**, and the two must move
...
[`ownership.md`](../ownership.md) has the rule. It never commits on
