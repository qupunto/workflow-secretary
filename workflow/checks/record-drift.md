...
[`record-contract.md`](../record-contract.md#negative-claims) before
...
**This method needs a file, and `record.todo` may not be one.** A provider-backed
...
[`providers/github-issues.md`](../providers/github-issues.md). Do not report the
absence as drift; it is the declared shape, not a fault.

### 3. Mutable claims in the tooling files

What `record.tooling.sources` — the skill and agent files — may and may not carry
is
...
Where the manifest names a check command (`commands.indexCheck`), run
...
live references, which is why they survive. The suite's `doctor.sh` checks this
