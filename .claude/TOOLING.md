...
`tooling-catalog-sync` (`--tools`) owns this file and is its sole writer.
`--full-check` hands it back here at the end of every health check, which is what
keeps it from becoming the stale inventory this workflow warns about everywhere
else.

**Where a project has a documentation site**, `--tools` hands this file to
`--docs`, which adapts it into a Claude-tooling annex page and owns that page.
...
say so. It used to need two: `--docs`, `--stocktake` and `--adopt` each still
...
it. Arrows also run the other way — the primitive `--tools` invokes `--docs` —
...
| Caller | Invokes | For |
|---|---|---|
| `--adopt` | `manifest-writer`, `--docs`, `git-writer` | writing the manifest it decided on; scaffolding a project that has no documentation; committing. Amendment mode — one key in an existing manifest — reaches `manifest-writer` without the detection phase |
...
| `release` | `--release` | Decides that a version ships, once the roadmap marks a milestone done, and asks before anything is published. The entry and the tag are written by the two primitives above |
...
| `tooling-catalog-sync` | `--tools` | Keeps this catalog current, hands it to `--docs` where a site exists, and deletes stale claims from skill and agent files |
...
| `wrap-task` | `--wrap` | Closes out a session — task list, the handoff through `handoff-writer`, the commits and push through `git-writer`, asks `--plan` whether a milestone just finished, reports where the project stands in the reply (backlog left, decisions nobody has made, the next milestone), and says when it is safe to clear |
...
|---|---|---|
| [`audit-writer`](../workflow/writers/audit-writer.md) | `record.audits` | Writes the audit log entry — what a stocktake examined, against which tree, and what it found, with its coverage block. Also the one-field `Outcome` update when remediation lands, which is why it is not part of `--stocktake` |
| [`behaviour-writer`](../workflow/writers/behaviour-writer.md) | `record.behaviour` | Writes the record of what the system does at runtime, by topic. Never *why* it does it, which is `--log`'s |
...
| [`git-writer`](../workflow/writers/git-writer.md) | commits and tags | Makes the commits, the tags and `--pullrequest`'s merge for every skill that may, so the rules that keep a commit, a merge or a push safe live in one file rather than in whichever caller remembered them |
...
`--full-check` and `--stocktake` borrowed rather than copied — but they did it by
...
`--start` and `--check` and `--full-check` read it, `--wrap` counts it. None of
...
and this repo used to have two. `prune-skills` and then `repo-health` both proved
general enough to be worth every project having, the second by being merged into
`--full-check` rather than moved beside it. What is left here is global.
...
| `CLAUDE.md` | Loaded into every session in every project — the checkout form only, because a plugin root's is never read as project context. Now the three contract paths, the doctor line and a pointer: what each contract governs moved to `workflow-contracts`, which owns it |
