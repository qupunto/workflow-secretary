...
| `ws-release` | `--ws-release` | Decides that a version ships — once `record.releases` marks a milestone done, or once it has declared an end to milestones and the release is maintenance on evidence — and asks before anything is published. The entry and the tag are written by the two primitives above |
...
| `ws-wrap` | `--ws-wrap` | Closes out a session — task list, the handoff through `handoff-writer`, the commits and push through `git-writer`, asks `--ws-plan` whether a milestone just finished, and from a lane worktree lands that lane on `branch.integration` by fast-forward — refused rather than forced, and withheld entirely when the wrap fired on an unfinished session. Reports where the project stands in the reply (backlog left, decisions nobody has made, the next goal and milestone), and says when it is safe to clear |
