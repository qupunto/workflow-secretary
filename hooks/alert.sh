...
# opts in with `--ws-alerts on`. The state lives in the user's CONFIG
...
STATE="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/alerts-on"
[ -f "$STATE" ] || exit 0
...
# alerts-on rather than in the shared temp dir: a fixed name in /tmp is
# another user's to own or pre-link on a multi-user box, and this suite
# refuses symlinks everywhere else it writes.
STAMP="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.ws-alert.stamp"
now=$(date +%s)
