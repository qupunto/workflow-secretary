...
## The `--ws-track` shorthand

`--ws-track` invokes this immediately, with no confirmation, and overrides the
...
|---|---|---|
| Schema / data model | models, fields, relations, enums, indexes | the project's schema sequence — validate, regenerate the client, typecheck, migrate, reseed. Where `.claude/workflow.json` names an `onSchemaChange` skill, that skill owns the exact order |
| API / endpoints | routes, handlers, services, request schemas | the full test suite (`commands.test`), then iterate on what fails |
...
- **The task list here** — this session's working plan. Ephemeral. Gone when
  the session ends.
- **The project's backlog** — durable pending work for the repo, `record.todo`
  in the project's manifest. `--ws-todo` owns it, alongside the reasoning in
  `record.decisions`.

If something surfaces that should outlive the session it belongs there, not
just here. And when the whole unit of work is approved and done, `--ws-wrap` closes
the list out.
