...
| `overview.md` | Stack tables, quickstart, scripts, env vars, directory tree, key decisions summary | Always |
...
| `decisions.md` or `decisions/` | Design choices with context, alternatives rejected, consequences (ADRs) | Always — inline list when few, one file per decision past ~10 |
...
| `stack.md` | Dependency inventory with the reason each was chosen | Only when the stack is large enough that `overview.md`'s tables get unwieldy |
...
**→ `references/tier-governance.md`** for the page table and the sector framework map
...
`annex/permissions.md`, `annex/storybook.md`, `annex/claude-tooling.md`.

`annex/claude-tooling.md` is the one annex whose source is another skill's record rather than
the codebase: `--ws-tools` owns `record.tooling.catalog` and hands it over, and this page is the
...
[`project-shape.md`](../../../workflow/project-shape.md)'s** — one detector, so
this skill and `--ws-stocktake` cannot reach different conclusions about the same repo.
