...
`/docs` invokes this skill by name, and `--docs` fires it through the hook — both are the same
...
picked before writing. With an argument (`--docs auth`, `--docs the map module`) that's the
target.

**`--adopt` also invokes this** when it finds a project with no documentation at all, wanting
...
| PR descriptions | `pr-flow` (`--pullrequest`), which drafts the body from the branch range. It is not a record and is never written to a file here |
...
| A project's *append-only* record — backlog, roadmap, decision log, audit log | The record primitives: `--todo`/`--log`, `--plan`, `--stocktake`. Appending a dated entry is not writing a page, and those files are never placed by tier |
...
is `--check`'s finding, not this skill's. Don't run both against the same request.
...
[Audit scope](#audit-scope) below, and `--full-check` is how you force every page to be
re-read.
...
5. **A page set larger than one session is state, and this skill stores none.** Hand the
   unwritten pages to `--todo` so they outlive the session in the project's backlog, and
   `--track` them within it. Never keep the plan only in the reply — on a large site that is
...
# recorded in the `workflow-contracts` skill.
S="${CLAUDE_CONFIG_DIR:-$HOME/.claude}"
[ -x "$S/doctor.sh" ] || S=$(ls -d "$S"/plugins/cache/*/workflow-secretary/*/ 2>/dev/null | tail -1)

bash "$S"/skills/docs/assets/scaffold.sh docs "<Project Name>" [root-lang [translation-lang ...]]
```

```bash
bash "$S"/skills/docs/assets/scaffold.sh docs "Acme UI"        # monolingual
bash "$S"/skills/docs/assets/scaffold.sh docs "SIME UI" en ca  # root English, docs/ca/ Català
```
