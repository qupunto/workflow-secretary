...
`assets/scaffold.sh` is the single source of truth for the generated shell — this page
...
├── overview.md         # ← you write this first, it is not scaffolded
...
to the dependency table in `overview.md` once that page exists.
...
| Section | Content |
|---|---|
| [Overview](docs/overview.md) | Tech stack, scripts, directory structure, key architectural decisions |
````
...
Write `overview.md` first — it forces an inventory of the stack, scripts, env vars, and
directory layout, which is the raw material every other page links back to.

1. `overview.md` — tech stack tables, getting started, scripts, env vars, directory tree,
