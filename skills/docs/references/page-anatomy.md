...
| File | Description |
|---|---|
| [overview.md](overview.md) | Tech stack, scripts, directory structure, key architectural decisions |
...
appear once a group holds two or more pages. `references/taxonomy.md` owns the full tier
...
Earns its place on `overview.md` and on any page whose subject is incomplete. It is often the
...
Project-wide gaps collect on `overview.md`. A gap local to one subsystem stays on that
