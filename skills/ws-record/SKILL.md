---
name: ws-record
description: "The project's record of work and why. `--ws-todo` parks what is not being built now: task to the backlog, reasoning to the decision log. `--ws-log` records a decision already made. Also on \"park this\", \"leave this for later\", \"log that we decided X\", pasted standup notes, or when you judge something premature. Not on a decision mentioned in passing."
---
...
| Flag | Means | Writes |
|---|---|---|
| `--ws-todo` | park this; we are not building it now | `record.todo` **and** `record.decisions` — a deferral is a decision |
| `--ws-log` | we decided this; record the reasoning | `record.decisions` only |
...
`commands.indexRegen`. Where a `.claude/lane` selector names a lane,
`lanes.named.<lane>.records.X` overrides `record.X` for `todo` and
`openDecisions` — [`manifest.md`](../../workflow/manifest.md)'s resolution
rule; `record.decisions` never splits, and under lanes it is fed by promotion,
per [`record-contract.md`](../../workflow/record-contract.md). The fallbacks
...
- **A date with no decision attached.** A deadline belongs to `--ws-plan`.
...
routing; the writing is `--ws-todo` and `--ws-log` exactly as above, under whatever
...
- **An explicit deferral must be marked, because there is no section to put it
  ...
  `[blocked → …]`. This is not optional bookkeeping: `--ws-start` reads issues
...
hand-edited, and a later `--ws-check` fails if you skip it. Where the manifest
...
- **`record.changelog` is the neighbour worth keeping straight.** `--ws-log` records
...
- **It does not judge whether the work is worth doing.** A known bug that will
  ...
  up.
- **It is not the session task list.** That is `--ws-track`: ephemeral, gone when
...
`--ws-start` when it settles an open decision or removes a shipped item, `--ws-stocktake`
when it dispositions a finding, `--ws-check` when it dispatches one.
