...
it has an index: run `commands.indexCheck` and dispatch a stale index to `--ws-todo`.

**Code and project position are out of scope.** The public interface, the safety
nets and the backlog rebuild are `--ws-full-stocktake`'s, and correctness, security
...
- **Docs site** — [`workflow/checks/docs-audit.md`](../../workflow/checks/docs-audit.md),
  every section, over every page — the incremental narrowing lives in `--ws-docs`
...
under its own rules. `--ws-full-check` grants nothing, so **an owner it invokes writes
...
**Run `ws-tools`'s prune job after the dispatch, not before:**
...
It reports and dispatches; it does not cut. `--ws-tools` makes the cut after its own
second look.

Then hand `record.tooling.catalog` to `--ws-tools`, which owns it: add, edit or
...
- **It does not audit source.** Running the project's own checks is not the same
  as reading its code; that is `--ws-full-stocktake`.
