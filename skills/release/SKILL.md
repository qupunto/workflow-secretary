...
- **Looks complete but is not marked** — do not tag it. Hand to `--plan`, which
  ...
  another skill's record, and it skips the two disqualifier checks `--plan` runs
...
**Invoke `--full-check`.** A tag is a claim that the tree it names is sound, and
...
`--full-check` rather than `--check`, deliberately. The incremental sweep trusts
...
precondition, and it is `--plan`'s file. Do not edit it here either.
