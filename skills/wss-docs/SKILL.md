...
| PR descriptions | `wss-pr` (`--wss-pr`), which drafts the body from the branch range. It is not a record and is never written to a file here |
...
# reaches hooks, never the Bash tool — is recorded in the `wss-contracts` skill.
S="${CLAUDE_CONFIG_DIR:-$HOME/.claude}"
[ -x "$S/wss-doctor.sh" ] || S=$(ls -d "$S"/plugins/cache/*/workflow-secretary-suite/*/ 2>/dev/null | tail -1)

bash "$S"/skills/wss-docs/assets/wss-scaffold.sh docs "<Project Name>" [root-lang [translation-lang ...]]
```

```bash
bash "$S"/skills/wss-docs/assets/wss-scaffold.sh docs "Acme UI"        # monolingual
bash "$S"/skills/wss-docs/assets/wss-scaffold.sh docs "SIME UI" en ca  # root English, docs/ca/ Català
```
