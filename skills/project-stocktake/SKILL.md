...
`record.*`; `agents.audit` for the fan-out and the remediation owners under
...
- No audit history, so the run is effectively `--full-stocktake`: skip Phase 0
...
5. **Under `--full-stocktake`, skip steps 3 and 4.** Everything is in scope,
...
8. **Create the task list up front**, per `track-complex-tasks`: one task per
...
- **Defer with reasoning** — hand to `--todo`: task to `record.todo`, reasoning
...
*decided* here and *written* by `--todo`, which owns that file. This skill
...
2. **Then rewrite it through `--todo`, and delegate the transcription.** Hand a
...
4. **`record.roadmap` if priorities moved — propose, don't write.** `--plan` is
   ...
   block; hand the reorder to `--plan` even when you have the whole picture and
   the edit looks trivial. **A finding about a version or a tag is `--release`'s
...
5. **`record.handoff`, narrowly — through `handoff-writer`, which owns it.** Hand
   ...
   currency pass comes later, when Phase 5's `--wrap` calls the same primitive.
6. **Hand any decision this audit produced to `--log`, and let it regenerate the
   index.** `record.decisions` and `record.decisionsIndex` are `project-record`'s,
...
**Invoke `--wrap`** — the whole ritual this time, not the narrow hand-off of
...
**`--stocktake` and `--full-stocktake` are standing authorization to commit and push**,
exactly as `--wrap` is, and it does not need asking again. The grant comes from
...
rules — commit it, then ask before pushing, or wait for `--wrap`.
...
- **It doesn't commit or push by hand, and never tags.** Its record goes out
  through `git-writer`. Deciding that a version ships is `--release`'s alone.
...
## It absorbs `--check`'s standalone sweep
