---
name: start-work
description: "Pick up the project's pending work and do it — settle the open decisions first, select a batch from the backlog, partition it into lanes that cannot collide, and run those lanes in parallel. SHORTHAND: `--start`. Also trigger on \"carry on with what's pending\", \"pick up the next thing\", \"keep going with the backlog\"."
---
...
gates it behind the user's own words (`commands.testConsentEnv`). Budget for
...
the decision log whole; go through `record.decisionsIndex`, pick the entries that
...
3. **Read the three planning records**, plus `record.decisionsIndex`.
4. **Create the task list up front** via `--track`: one task per open decision,
...
Every entry in `record.openDecisions`, in file order — it is already ordered by
...
For each decision the user actually makes, **hand it to `--log`** — which owns
...
that goes through `--todo`, which owns the task-and-reasoning split.
...
`provider` key, per
[`providers/github-issues.md`](../../workflow/providers/github-issues.md). Read
...
one. When an item lands, closing it is `--todo`'s, the same as deleting a line.
...
- **Deferral is a marker, not a place.** The "explicitly deferred" exclusion
  ...
  filed a minute ago by `--todo` may well be one that was *just* deferred —
...
- **Versions and tags** — `--release`'s decision alone, and the changelog entry
...
next-up. Do not improvise its breakdown here: hand the block to `--plan`, which
...
| **Every `record.*` file** | **No lane writes these.** Every lane will want to and they would collide on every line. The orchestrator records once, in Phase 6, through the owning primitives. |
...
|---|---|
| Schema shape, service boundaries, "how should this be modeled" | `agents.architecture` first (advises only), then the lane that implements it |
| Endpoints, services, domain logic, queries | `agents.implement` |
| Containers, deployment and build config, operational scripts, CI | `agents.infra` |
| Test coverage, the authorization matrix, validation boundaries | `agents.test` |
| Proving a specific vulnerability before anyone fixes it | `agents.exploit` — rare and expensive, against a named surface only |
...
- **Whether it may run the test suite at all.** Where the project gates the suite
  ...
  entirely within its own scope. Where the manifest names one under `hazards.*`,
...
- **Run the grep that would disprove a negative claim** before reporting "nothing
  does X" — [`record-contract.md`](../../workflow/record-contract.md#negative-claims).
...
2. **`commands.typecheck`.** Cross-lane type breakage lives here and nowhere
...
4. **Ask for suite consent, then run `commands.test` once.** Use the coverage
...
5. **Stamp the run through `sweep-tracker`**, as the `test-run` entry: the sha
   ...
   stamp at all**: `--stocktake` skips its own run on the strength of this entry,
   so one no run earned is worse than none. A red result *is* stamped, as red —
   [`sweep-checkpoint.md`](../../workflow/sweep-checkpoint.md) has the fields and
...
1. **`--todo`** — remove what shipped, and rewrite anything that turned out
   bigger than its checkbox with what was actually learned. Completed items
   *leave*; they are not kept struck through.
2. **`--log`** — any non-obvious call a lane made while building. Phase 1's
   decisions are already recorded.
3. **`behaviour-writer` and `reference-writer`** — what the *code* changed:
   `record.behaviour` for runtime rules, `record.reference` for schema and
   architecture. Straight to the owning primitive, not through `--docs`, which
   owns the site and no longer writes either record. Invoke `--docs` separately
   where the change also earns a page.
4. **`--plan`** — only if a block advanced or completed. `--plan` is what asks
...
5. **New open decisions.** A batch that surfaced a choice and settled it silently
   has done the thing Phase 1 exists to prevent. Route it through `--todo`, to the
   open-decisions record if still open.
6. **`--tools`** if any lane touched a skill or agent file.
7. **`handoff-writer`** — what the batch changed, plus any `!important` it
   created or resolved, so the next session inherits it. Not `--wrap`: this skill
   grants commit and not push, and an invoked skill inherits the caller's grant,
   so calling the whole closing ritual here would end the session on `--start`'s
   authority rather than the user's. The user typing `--wrap` is what gets the
   ritual and the push.

## What this skill does not do

- **It does not push, and nothing it invokes may push on its behalf.** `--start`
...
- **It does not review existing code.** Finding what is wrong with what is already
  there is `--stocktake`; this skill builds what is not there yet.
- **It does not invent documents.** If nothing fits, say so rather than inventing
  structure — `record-contract.md`'s standing rule, and it holds here.
