...
`--ws-adopt` (`ws-adopt`), which writes the manifest.
...
- **It writes nothing** — no record, no manifest, no settings. `--ws-adopt` writes a
  manifest; the record writers own the records.
- **It does not decide anything.** A session that needs a choice made goes to the
  project's own open-decisions record, which `--ws-todo` owns.
