...
`--stocktake` calls the whole ritual to close out an audit. That is the one
sanctioned case, and it changes three things:

- **The grant is the caller's, at the caller's scope.** `--stocktake` grants
...
- **Do not declare the session safe to `/clear`.** Step 7 is written for a user
  ...
  else's procedure — `--stocktake` routes its Fix-now dispositions *after* this
...
6. **Check whether this session finished a milestone.** Read `record.roadmap`:
   ...
   milestone, **invoke `roadmap`** so the question is put to the user now, while
   the evidence is in front of them. Do **not** mark it completed yourself —
   that file is `--plan`'s, and whether a milestone is done is the user's call,
   which `--plan` asks and the two disqualifier checks it runs are part of.
   If the user marks it, name `--release` as the next step; if they don't, that
   ...
   **A mark written here lands after step 5's commit**, so commit it too rather
   ...
   It is `--plan`'s write and this skill's commit, which is the normal division.
...
`--wrap` is standing authorization to commit and push — it does not need
...
- **How the work divides.** You lived through it; tell `git-writer` which files
  ...
  diff.
- **Interrupted work still gets committed.** When `--wrap` fires mid-task,
...
**Where the pushed branch is now ahead of `branch.publish`, name `--pullrequest`
