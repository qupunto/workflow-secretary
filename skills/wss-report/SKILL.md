...
The upstream repository is **`qupunto/workflow-secretary-suite`** — hardcoded, like
...
4. **Send**, labelled where the label exists:
   ...
   ```bash
   gh label list --repo qupunto/workflow-secretary-suite --json name \
     --jq '.[] | select(.name == "report") | .name'
   gh issue create --repo qupunto/workflow-secretary-suite \
