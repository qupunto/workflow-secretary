---
name: ws-pr
description: "Move work from the integration branch to the publish branch through a pull request — draft the body from the actual range, open it, watch its CI, and merge once the user confirms. SHORTHAND: `--ws-pr`. Also trigger on \"open a PR\", \"raise a pull request\", \"is the PR green yet\", \"merge that PR\"."
---
...
- **Local commits not on the remote** — stop. Pushing is not this flag's grant,
  ...
  see. Say what is unpushed and let the user push, or `--ws-wrap`.
...
- Name the decision entries in `record.decisions` that this range implements, so
...
Query `commands.ci` for the PR's run. Where the manifest declares none, say that
...
- **Still running** — say so and stop, with the run's URL. Do not poll a long
  pipeline inside the turn; the user can come back to `--ws-pr`, which
...
**A red base is not a red PR.** Where `branch.publish` was already failing before
...
Then hand the merge to `git-writer` with the method from `branch.mergeMethod`,
...
Deleting the remote branch afterwards is only correct where `branch.integration`
...
go to `--ws-todo`, which owns the backlog and writes it wherever this project
...
Say what merged, at which commit on `branch.publish`, and what the range
contained. Then check whether it made anything else due:

- **A milestone whose last block just landed** — that is `--ws-plan`'s question to
  put to the user, not an inference from a merge.
- **A version worth tagging** — name `--ws-release` as the next step. This skill
  never tags; `branch.publish` moving is not a release.
...
- **It does not tag, and does not end the session.** A version is the release
  flag's, on `branch.publish` and behind a milestone mark rather than a merge;
  the closing ritual is `--ws-wrap`'s.
