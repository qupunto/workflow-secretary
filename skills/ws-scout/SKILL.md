...
**Project facts come from `.claude/workflow.json`**: `record.toolbelt` is the
registry, fallback `toolbelt.md`. An absent file is an empty registry, not an
...
[`record-contract.md`](../../workflow/record-contract.md) requires:

- **The row, here.** Task shape → package → pointer into `record.decisions`.
  Lean — the registry is a lookup table, not a second decision log.
- **The reasoning, through `--ws-log`** (`ws-record`): what was searched, what
...
([`record-contract.md`](../../workflow/record-contract.md)). Where the registry
pointer names a decision entry, use its heading, which is stable in an
append-only log; where the manifest declares `record.decisionsIndex`, the index
...
- **It does not write the reasoning.** That is `ws-record`'s file, reached
  through `--ws-log`.
