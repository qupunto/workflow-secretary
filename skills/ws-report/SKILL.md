---
name: ws-report
description: "File a finding about this suite upstream — append it to the machine-local inbox, then, with an explicit OK in that turn, open a GitHub issue on the suite's public repository. SHORTHAND: `--ws-report`. Also trigger on \"report this upstream\", \"file this against the suite\", \"send this to the workflow repo\"."
---

# Reporting a suite finding upstream

The inbox flow this mechanizes already exists:
[`ownership.md`](../../workflow/ownership.md)'s cross-project rule files every
suite finding into the machine-local `bug-reports.md` and stops, and an adopter
...
The upstream repository is **`qupunto/workflow-secretary`** — hardcoded, like
...
1. **Append the entry to the inbox** — `$CLAUDE_CONFIG_DIR/bug-reports.md`
   (`~/.claude/bug-reports.md` unless that variable is set), below the append
...
4. **Send**, labelled where the label exists:
   ...
   ```bash
   gh label list --repo qupunto/workflow-secretary --json name \
     --jq '.[] | select(.name == "report") | .name'
   gh issue create --repo qupunto/workflow-secretary \
...
`reset-records.sh` blanks out of the published tree, which is the measure of
how private it is. Where a finding relates to a documented hazard, name the
group and the manifest key that points at it (`hazards.plugin`, say): the
...
- **Not for project findings.** A defect in the project you are working on
  goes to that project's own records through `--ws-todo`; this skill is only
