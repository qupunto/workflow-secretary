...
| `--wss-stocktake` | `--wss-todo` / `--wss-log`, `--wss-plan`, `--wss-tools`, `--wss-wrap`, `audit-writer`, `handoff-writer`, `git-writer`, `sweep-tracker`, and the project's own code-analysis skill where one exists | the dispositions, its audit entry, and a dispatched close-out. It runs the record dimension itself from `workflow/checks/WSS.RECORD-DRIFT.md` — the hook drops `--wss-check` when either stocktake flag is typed |
